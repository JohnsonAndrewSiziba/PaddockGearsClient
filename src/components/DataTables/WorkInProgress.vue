<template>
  <div>
    ...
    <table class="compact  order-column wip-jobs" style="width: 100%; font-size: 11px; line-height: 12px; font-weight: lighter">
      <thead>
      <tr>
        <th>Date</th>
        <th>Job #</th>
...
export default {
  ...
  methods: {
    ...
    renderTable(){
      $(function () {
        var table = $('.wip-jobs').DataTable({
          processing: true,
          serverSide: true,
          pageLength: 50,
          dom: 'Bfrtip',
          ajax: API_BASE + "api/work_in_progress",
          columns: [
            {data: 'date', name: 'date'},
            {data: 'id', name: 'id'},
